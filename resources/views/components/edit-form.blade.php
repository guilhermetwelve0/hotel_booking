<div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
    <form class="max-w-xl" action="{{ route("$route.update", $data->id) }}" method="post" enctype="multipart/form-data">
        ...
        <div class="pb-5">
            ...
            <textarea name="{{$name}}" rows="4" class="block p-2 w-full text-sm mt-1 text-gray-900 bg-white rounded-md border border-gray-300 focus:ring-blue-500 focus:border-blue-500">{{$data->$name}}</textarea>
        ...
        <div class="pb-5">
            ...
            <input name="{{ $name }}" type="{{ $type }}" value="{{$data->thumbnail}}" class="img-upload block w-full text-md text-gray-900 border border-gray-300 rounded-md cursor-pointer bg-white focus:outline-none">
        ...
        <div class="pb-5">
            ...
            <select id="multiple-select" multiple name="{{$name}}[]" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-md focus:ring-blue-500 focus:border-blue-500 block w-full">
        ...
        <div class="pb-5">
            <x-input-label for="{{ $name }}" :value="$label" :required="$required" />
            <x-text-input id="{{ $name }}" name="{{ $name }}" type="{{ $type }}" class="mt-1 block w-full" autocomplete="{{ $autocomplete }}" :value="old($name, $data->$name)" :required="$required" :autofocus="$autofocus" />
            <x-input-error class="mt-2" :messages="$errors->get($name)" />
        </div>
        @endswitch
        @if ($name == "price")
